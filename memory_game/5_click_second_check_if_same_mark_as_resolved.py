...
def draw_board(display, board, resolved_boxes_matrix, clicked_boxes):
    BOXSIZE= 39
    for row in range(1,10):
        for col in range(1,10):
            if (row, col) in clicked_boxes or resolved_boxes_matrix[row][col]==1: #if the box was clicked or resolved display the game piece
    ...
    while running:
        ...
        if boxx != None and boxy != None: # This means the mouse is over a box
            if mouseClicked:
                ...
                if(len(clicked_boxes)==2):
                    first_box_clicked = clicked_boxes[0]
                    game_piece_in_first_box = board[first_box_clicked[0]][first_box_clicked[1]]
                    second_box_clicked = clicked_boxes[1]
                    game_piece_in_second_box = board[second_box_clicked[0]][second_box_clicked[1]]
                    if(game_piece_in_first_box == game_piece_in_second_box):
                        resolved_boxes_matrix[first_box_clicked[0]][first_box_clicked[1]] = 1 # 1 means the box was resolved
                        resolved_boxes_matrix[second_box_clicked[0]][second_box_clicked[1]] = 1

        draw_board(DISPLAYSURF, board, resolved_boxes_matrix, clicked_boxes)
